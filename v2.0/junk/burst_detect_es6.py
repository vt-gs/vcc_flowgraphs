...
class burst_detect_es6(gr.top_block, Qt.QWidget):
    ...
    def set_samps_per_symb(self, samps_per_symb):
        ...
        self.digital_clock_recovery_mm_xx_0.set_omega(self.samps_per_symb*(1+0.0) / self.decim_2)
...
def argument_parser():
    ...
    parser.add_option(
        "", "--lpf-cutoff-2", dest="lpf_cutoff_2", type="eng_float", default=eng_notation.num_to_str(8e3),
...
def main(top_block_cls=burst_detect_es6, options=None):
    if options is None:
    ...
    tb = top_block_cls(lpf_cutoff=options.lpf_cutoff, lpf_cutoff_2=options.lpf_cutoff_2, lpf_trans=options.lpf_trans)
    tb.start()
    tb.show()
